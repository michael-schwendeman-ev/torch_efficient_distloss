...
class FlattenEffDistLoss(torch.autograd.Function):
    ...
    def forward(ctx, w, m, interval, ray_id):
        ...
        loss_bi = 2 * w * (m * w_prefix - wm_prefix)
        if torch.is_tensor(interval):
            ctx.save_for_backward(w, m, w_prefix, w_total, wm_prefix, wm_total, ray_id, interval)
            ctx.interval = None
        else:
    ...
    def backward(ctx, grad_back):
        ...
        n_rays = ctx.n_rays
        if interval is None:
            w, m, w_prefix, w_total, wm_prefix, wm_total, ray_id, interval = ctx.saved_tensors
        else:
